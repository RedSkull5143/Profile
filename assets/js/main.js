...
const setExperience = (experiences) => {
  ...
  experiences.forEach((exper) => {
    ...
    expItem.style.flexDirection = "column"; // Stack inner elements

    // 🔹 First line: Organization + Title + Duration
    ...
    const orgTitleWrap = document.createElement("div");
    orgTitleWrap.style.display = "flex";
    orgTitleWrap.style.flexWrap = "wrap";
    orgTitleWrap.style.gap = "6px";
    ...
    orgTitleWrap.appendChild(expOrg);
    orgTitleWrap.appendChild(expTitle);

    const expDura = document.createElement("span");
    expDura.className = "exp-duration";
    expDura.innerText = exper.date;

    firstLine.appendChild(orgTitleWrap);
    firstLine.appendChild(expDura);
    expItem.appendChild(firstLine);

    // 🔹 Client (Optional)
    if (exper.client) {
      const expClient = document.createElement("div");
      expClient.className = "exp-org";
      expClient.style.fontSize = "0.9em";
      expClient.style.color = "#666";
      expClient.style.marginTop = "2px";
      expClient.innerText = `Client: ${exper.client}`;
      expItem.appendChild(expClient);
    }
    ...
    if (exper.details) {
      ...
      exper.details.forEach((detail) => {
        const li = document.createElement("li");
        li.innerHTML = detail;
        expDetails.appendChild(li);
      });

      expItem.appendChild(expDetails);
    }
